...
2. 半虚拟化（Paravirtualization）：利用宿主机提供的 API 接口模拟运行环境，其上运行的代码需要针对宿主机平台特别定制。

很多现代 CPU 都会提供特殊的指令以便加速虚拟化过程，比如 Intel 的 VT-x 和 AMD 的 AMD-V；新一些的 CPU 还会提供对 IO 设备进行虚拟化的功能，比如 Intel 的 VT-d，利用这些功能可以极大提升硬件虚拟化的效率。

#### 2.3.2 操作系统级虚拟化

操作系统级虚拟化一般也被称为容器化，是指在操作系统的层面对不同的用户空间进行隔离的技术。被分离的用户空间一般被称为容器。容器化的

### 2.4 Docker

Docker 是常见的容器化技术之一，属于上述操作系统级虚拟化的一个比较典型的应用。
